...
module InlineListing = {
  ...
  type instanceVars = {
    mutable preRef: option ReactRe.reactRef
  };
    ...
    >
      <div className onClick=(fun e => e##stopPropagation ())>
        <div className="sidebar">
          (ReactRe.stringToElement props.lang)
        </div>
        <div className="main">
          ...
          <footer>
            <CopyButton text=props.text onCopy=(fun () => ()) />
            <OpenButton onClick=(fun _ => props.open_ props.text) />
          </footer>
        </div>
      </div>
    </Transition>
