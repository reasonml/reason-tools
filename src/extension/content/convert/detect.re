...
let hasOcamlRels = () => {
  let hits =
    ...
    |> List.map((link) => ocamlishRels |> Js.Array.includes(Element.getAttribute(link, "rel")))
    |> List.filter(id)
    |> List.length;
  hits >= 3
};
...
let isWhitelisted = () => getWhitelist() |> List.mem(getSignificantUrl());
...
let shouldConvert = () => {
  ...
  | None =>
    ...
    let shouldConvert = isWhitelisted || mightBeOcamlDoc && ! isBlacklisted;
    /*
