...
let replace this => Js.String.replaceByRe (Js.Re.fromString ("/\\b" ^ escapeRe this ^ "\\b/g"));

let replaceAll = List.fold_left (fun out (this, that) => out |> replace this that);

let getNormalizedLinks els =>
  els |> List.map (fun el => getElementsByTagName (Some el) "a") |> List.flatten |>
  List.map (fun a => (Element.innerText a, Element.href a));

let getNormalizedIds els =>
  els |> List.map (fun el => querySelectorAll el "[id]") |> List.flatten |>
  List.map (
    fun span => {
